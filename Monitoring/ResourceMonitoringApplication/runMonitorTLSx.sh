...
rm -f $tlsYaml
cat $OUTDIR/$monitor.yaml | sed "s/clientConnection: false/clientConnection: true/g" > $tlsYaml  
chmod 555 $tlsYaml

# Create the ccdt file with a suitable address as "localhost" is problematic in containers
ccdtJson=$OUTDIR/ccdt.json
rm -f $ccdtJson
cat /Users/path/to/ccdt/json/file/connection_info_ccdt.json | sed "s/localhost/$addr/g" > $ccdtJson
chmod 555 $ccdtJson
...
runmqakm -keydb -create -db $OUTDIR/ssl/key.kdb -pw passw0rd -type pkcs12 -expire 0 -stash
chmod +rw $OUTDIR/ssl/key.*
...
cp /Users/path/to/public/certificate/cloud/file/mq-ldap/qmgrcert.pem $OUTDIR/ssl/.
runmqakm -cert -add -db $OUTDIR/ssl/key.kdb -file $OUTDIR/ssl/qmgrcert.pem -label qmgrcert -stashed -type pkcs12 -format ascii
rm -f $OUTDIR/ssl/qmgrcert.pem
...
docker run --rm -p 9157:9157 \
              ...
              -e IBMMQ_CONNECTION_USER="cloudMQuser" \
              -e IBMMQ_CONNECTION_PASSWORD="pOyYnMhoWrpassw0rd8888wXxXxXxXUGxT0" \
              -it $TAG:$VER
